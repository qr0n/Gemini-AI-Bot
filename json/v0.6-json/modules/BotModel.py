...
class BotModel:
    ...
    def generate_content(prompt, user_id=None, attachment=None, retry=3):
        ...
        print("Called function BotModel.generate_content() [Line 119 from modules/BotModel.py]")
        ...
        try:
            ...
            # if it gets cooked
            print("Exception in function BotMode.generate_content() [Line 134 from modules/BotModel.py]")
            print(error)

            fallback_message = response.candidates[0].content.parts[0].text.strip()
            context_window[user_id].append(f"{character_name}: {fallback_message}")
            # gets the first response candidate and sends it off to the dictionary
